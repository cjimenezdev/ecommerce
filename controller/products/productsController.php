<?php
	...
	class productoControlador extends mainModel{
        ...
        public function registrar_producto_controlador(){
            ...
            $producto_genero=mainModel::limpiar_cadena($_POST['producto_genero_reg']);
            $producto_tipo_persona=mainModel::limpiar_cadena($_POST['producto_tipo_persona_reg']);
			...
			$img_dir='../public/assets/img/products/cover/';
            ...
            $datos_producto_reg=[
                ...
                "producto_genero"=>[
					...
					"campo_valor"=>$producto_genero
				],
                "producto_tipo_persona"=>[
					"campo_marcador"=>":TipoPersona",
					"campo_valor"=>$producto_tipo_persona
				],
        ...
        public function administrador_paginador_producto_controlador($pagina,$registros,$url,$busqueda){
            ...
            if($tipo=="product-category"){
                $url=URL.DASHBOARD."/".$url."/".$busqueda."/";
			}else{
            ...
            if($total>=1 && $pagina<=$Npaginas){
				...
				foreach($datos as $rows){
					...
					$tabla.='<div class="card p-3">
                        <div class="row mb-4 mt-4">
                            <div class="col-12 col-md-4">
                                <figure> <div class="card-body d-flex justify-content-center align-items-center">';
                                if(is_file("./public/assets/img/products/cover/".$rows['producto_portada'])){
                                    $tabla.='<img src="'.URL.'public/assets/img/products/cover/'.$rows['producto_portada'].'" class="img-fluid" alt="'.$rows['producto_nombre'].'" style="height:200px;" />';
                                }else{
                            ...
                            <div class="col-12 col-md-8 d-flex flex-column justify-content-center align-items-center">
                                <h5 class="mb-1 roboto-condensed-regular text-center mb-4">'.$contador.' - '.$rows['producto_nombre'].'</h5>
                                ...
                                <div class="d-flex ms-md-auto">
                                    <div class="btn-group shadow-0">
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            if($_SESSION['cargo_sto']=="Administrador"){
                                                $tabla.='<li>
                                                    <a class="dropdown-item" href="'.URL.DASHBOARD.'/product-cover/'.mainModel::encryption($rows['producto_id']).'/" >
                                                        <i class="far fa-image"></i> &nbsp; Foto o portada
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item" href="'.URL.DASHBOARD.'/product-gallery/'.mainModel::encryption($rows['producto_id']).'/" >
                                                        <i class="far fa-images"></i> &nbsp; Galería de imágenes
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item" href="'.URL.DASHBOARD.'/product-update/'.mainModel::encryption($rows['producto_id']).'/" >
                                                        <i class="fas fa-sync-alt"></i> &nbsp; Actualizar datos
                                                    </a>
                                                </li>
                                                <li><hr class="dropdown-divider" /></li>
                                                <li>
                                                    <form class="FormularioAjax" action="'.URL.'ajax/productoAjax.php" method="POST" data-form="delete" data-lang="'.LANG.'" >
			...
			if($total>0 && $pagina<=$Npaginas){
				$tabla.='<hr><div class=" p-3 d-flex flex-column flex-md-row justify-content-between align-items-center"><p class="text-start">Mostrando productos <strong>'.$pag_inicio.'</strong> al <strong>'.$pag_final.'</strong> de un <strong>total de '.$total.'</strong></p>';
			}
        ...
        public function caballero_paginador_producto_controlador($pagina,$registros,$url,$orden,$categoria,$busqueda){
            ...
            if(isset($busqueda) && $busqueda!=""){
				...
				$consulta="SELECT SQL_CALC_FOUND_ROWS $campos FROM producto WHERE producto_estado='Habilitado' AND producto_genero = 'Masculino' AND producto_tipo_persona = 'Caballero' AND producto_stock>0  ORDER BY $campo_orden LIMIT $inicio,$registros";
			}
        ...
        public function dama_paginador_producto_controlador($pagina,$registros,$url,$orden,$categoria,$busqueda){
            ...
            if(isset($busqueda) && $busqueda!=""){
				...
				$consulta="SELECT SQL_CALC_FOUND_ROWS $campos FROM producto WHERE $condicion_busqueda producto_estado='Habilitado' AND producto_genero = 'Femenino' AND producto_tipo_persona = 'Dama' AND producto_stock>0 AND producto_nombre  LIKE '%$busqueda%' ORDER BY $campo_orden LIMIT $inicio,$registros";
			}elseif($categoria!="all"){
				$consulta="SELECT SQL_CALC_FOUND_ROWS $campos FROM producto WHERE producto_estado='Habilitado' AND producto_genero = 'Femenino' AND producto_tipo_persona = 'Dama'  AND categoria_id='$categoria' AND producto_stock>0 ORDER BY $campo_orden LIMIT $inicio,$registros";
			}else{
				$consulta="SELECT SQL_CALC_FOUND_ROWS $campos FROM producto WHERE producto_estado='Habilitado' AND producto_genero = 'Femenino' AND producto_tipo_persona = 'Dama' AND producto_stock>0  ORDER BY $campo_orden LIMIT $inicio,$registros";
			}
		...
		public function agregar_galeria_producto_controlador(){
			...
			/*-- Directorios de imagenes - Image Directories --*/
			$img_dir='../public/assets/img/products/gallery/';
